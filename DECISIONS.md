...
35f. **链路延迟记录**（真实问题：Signal 离线、Claude 请求失败时，要花很长时间才能确认是节点在某个时段变慢，
    而链路探测的结果用完就被覆盖）：每次真正发出的链路探测追加一行 JSON 到 `~/.local/state/proxyctl/chain-history.jsonl`
    （`$XDG_STATE_HOME`，重启后仍在；`XDG_RUNTIME_DIR` 是 tmpfs，重启即丢）。定时器复用缓存结果时不记录，所以正常时约 2 分钟一条、
    异常时 15 秒一条，异常时段自然更密。以 root 运行时不记录，免得写进 /root 而 GUI 读不到。
    每条记录带上当时监听 SOCKS 端口的进程名（用户在 sing-box 和 xray 之间切换，切换前后延迟特性不同）；看不到监听进程时退回正在运行的代理核心。
    文件超过 256 KB 且最早一条已超过 7 天才重写一次，避免每次追加都重写。
    异常时段：连续 2 次以上失败或超过 slow_seconds 才算，单次抖动不列出；相邻两条记录间隔超过 10 分钟（休眠、关机）时断开，曲线也在此断开。
    GUI 曲线用 GTK4 的 GSK 路径（Gsk.PathBuilder，GTK 4.14 起）绘制，不用 cairo：PyGObject 画 cairo 需要另装 python3-gi-cairo，
    作者本机就没有装。GTK3 或更老的 GTK4 只显示文字说明和异常列表。纵轴上限按最慢一次取整，最多 20 秒，超出的点压在顶部；
    失败画成顶部的红色 ✕，不参与纵轴计算。
    内核用固定颜色（sing-box 绿、xray 橙、mihomo 紫，按名字包含关系匹配），颜色跟着内核走而不是按出现顺序分配；
    三色经 dataviz 校验器验证（浅色 / 深色、所有两两组合都色盲可分）。文字始终用正文色，只把标识色作为底色，
    浅色主题下绿色对比度不足 3:1，但每块都写着内核名，不单靠颜色区分。
